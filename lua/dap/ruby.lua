...
local function run_cmd(cmd, args, for_current_line, for_current_file, error_on_failure)
	...
	local opts = { args = args, cwd = working_dir, stdio = { nil, stdout, stderr } }
	...
	if not handle then
		stdout:close()
		stderr:close()
		error("Error running command: " .. cmd .. tostring(pid_or_err))
	end
		...
		return function(err, chunk)
			...
			if chunk then
				vim.schedule(function()
					require("dap.repl").append("[ruby-adapter " .. stream .. "] " .. chunk)
				end)
			else
				pipe:close()
			end
...
local function setup_ruby_adapter(dap)
	dap.adapters.ruby = function(callback, config)
		...
		if config.command then
			...
			-- vim.env.RUBY_DEBUG_POSTMORTEM = "1"
...
local function setup_ruby_configuration(dap)
	local base_config = { type = "ruby", request = "attach", options = { source_filetype = "ruby" }, error_on_failure = false, localfs = true }
